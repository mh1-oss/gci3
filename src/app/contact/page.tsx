      ...
      <AnimatePresence mode="wait">
        {!settings ? (
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-brand-light flex flex-col items-center justify-center font-arabic z-50"
          >
            <div className="w-16 h-16 border-4 border-brand-red/20 border-t-brand-red rounded-full animate-spin mb-4" />
            <p className="text-brand-navy font-bold animate-pulse text-lg">تحميل بيانات التواصل...</p>
          </motion.div>
        ) : (
          <motion.div 
            key="content"
